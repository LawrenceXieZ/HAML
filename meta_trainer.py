...
class MetaTrainer(Trainer):
    ...
    def train(self):
        ...
        for e in range(self.args.metatrain_num_epoch):
            for batch in self.train_subgraph_dataloader:
                ...
                for batch_i, data in enumerate(batch):
                    ...
                    # Manhattan区域共13个子区域，额外循环一次。
                    additional_loop_times = 13
                    inner_loss_sum = 0
                    for loop_idx in range(additional_loop_times):
                        temp_loss = self.get_loss(que_tri, que_neg_tail_ent, que_neg_head_ent, ent_emb)
                        self.logger.info(f'Additional loop {loop_idx + 1}, temp loss: {temp_loss.item()}')
                        inner_loss_sum += temp_loss

                    # 计算平均损失作为更精准的损失值
                    loss = inner_loss_sum / additional_loop_times
                    # kge loss
                    loss = loss + self.get_loss(que_tri, que_neg_tail_ent, que_neg_head_ent, ent_emb)
        ...
        self.logger.info('best test | mrr: {:.4f}, hits@1: {:.4f}, hits@5: {:.4f}, hits@10: {:.4f}'.format(
    ...
    def evaluate_valid_subgraphs(self):
        ...
        self.logger.info('test on test subgraphs')
